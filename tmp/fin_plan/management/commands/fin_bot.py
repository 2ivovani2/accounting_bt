...
async def ask_for_invite_activation(update: Update, context: CallbackContext):
    usr, _ = await user_get_by_update(update)
...
async def activate_invite_code(update:Update, context: CallbackContext):
    usr, _ = await user_get_by_update(update)
    ...
    if code_in_base:
        if code_in_base.valid:
            try:
                ...
                await context.bot.send_message(
                    usr.telegram_chat_id,
                    f"✅ Вы успешно активировали инвайт код. Чтобы начать пользоваться ботом, нажмите на кнопку ниже.",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton(
                            text="Продолжить 💡",
...
@check_user_status
async def ask_for_type_of_invite_code(update: Update, context: CallbackContext):
    usr, _ = await user_get_by_update(update)
...
@check_user_status
async def ask_for_project_to_invite(update: Update, context: CallbackContext):
    usr, _ = await user_get_by_update(update)
...
@check_user_status
async def create_invite_code(update: Update, context: CallbackContext):
    usr, _ = await user_get_by_update(update)
...
def main() -> None:
    ...
    application = Application.builder().token(os.environ.get("FIN_BOT_TOKEN")).build()
    application.add_handler(CommandHandler("start", start))
    
    application.add_handler(ConversationHandler(
        entry_points=[CallbackQueryHandler(ask_for_type_of_invite_code, "create_invite_code")],
        states={
            0: [CallbackQueryHandler(ask_for_project_to_invite, "^choose_")],
            1: [CallbackQueryHandler(create_invite_code, "^project_choose_")]
        },
        fallbacks=[CallbackQueryHandler(start, "menu"), CommandHandler("start", start)]
    ))

    application.add_handler(ConversationHandler(
        entry_points=[CallbackQueryHandler(ask_for_invite_activation, "invite_code")],
        states={
            0: [MessageHandler(filters.TEXT, activate_invite_code)],
        },
        fallbacks=[CallbackQueryHandler(start, "menu"), CommandHandler("start", start)]
    ))
